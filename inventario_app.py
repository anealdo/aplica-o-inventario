...
arquivo_saldos = st.file_uploader("Carregar arquivo de saldos salvos (.xlsx)", type=["xlsx"], key="upload_saldos")
if arquivo_saldos:
...
uploaded_file = st.file_uploader("Faça upload do arquivo de inventário do sistema (formato .xlsx)", type=["xlsx"], key="upload_inventario")
if uploaded_file:
    ...
    with col1:
        if st.button("Adicionar ao saldo", key="btn_adicionar"):
    ...
    with col2:
        if st.button("Retirar do saldo", key="btn_retirar"):
    ...
    if st.button("Gerar Relatório Inventário", key="btn_gerar_relatorio"):
        ...
        if prateleira_selecionada != "Todas":
            df_relatorio = df_relatorio[df_relatorio["Prateleira"] == prateleira_selecionada]

        if st.checkbox("Mostrar apenas produtos com divergência", key="filtro_divergencia"):
        ...
        idx_divergencia = colunas.index("Divergencia") + 1
    ...
    if st.button("Baixar Saldos Físicos Atualizados", key="btn_baixar_saldos"):
